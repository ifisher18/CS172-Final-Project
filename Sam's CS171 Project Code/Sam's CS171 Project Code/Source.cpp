...
int command();
void canMove(int& n);
void map(int y, int x); // displays current map
...
int command() {
	...
	cin >> command;
	if (command == "map") { // displays current map to user
		map(x, y);
	}
	else if (command == "north") { // north command
		y = y - 1;
		canMove(y);
	}
	else if (command == "south") { // south command
		y = y + 1;
		canMove(y);
	}
	else if (command == "east") { // east command
		x = x + 1;
		canMove(x);
	}
	else if (command == "west") { // west command
		x = x - 1;
		canMove(x);
	}
...
void canMove(int& n) {
	if (n < 0) { // checks if n val is less than 0...the user is at edge of the map
